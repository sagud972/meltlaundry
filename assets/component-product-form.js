...
class VariantSelects extends HTMLElement {
  constructor() {
    ...
    if(this.formType == 'product-page') this.onVariantChange();
  ...
  _getOptionsFromRadio() {
    ...
    const options = fieldsets.map((fieldset) => {
      console.log(fieldset.querySelectorAll('input:checked'))
  ...
  renderProductInfo(currentVariant, container) {
    ...
    if(this.formType == 'grid'){
      ...
      if(this.formType == 'quickshop'){
        imageSlide = document.querySelector(`.quickshop-slider .product--image[data-imageID="${imageID}"]`);
      }
