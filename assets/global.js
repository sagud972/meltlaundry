...
class ModalDialog extends HTMLElement {
  constructor() {
    ...
    this.modalOpener = document.querySelector('[data-modal-opener="'+this.id+'"]');
    if(this.modalOpener) this.modalOpener.addEventListener('click', ()=>{
      this.show(this.modalOpener);
    });
 ...
 class sliderElement extends HTMLElement{
  constructor() {
    ...
    if(sliderElement.classList.contains('pdp-carousel')){
      ...
      const zoomType = sliderElement.dataset.zoomtype || 'click';
      if(zoomType == 'mouseover'){
        const swiperSlide = sliderElement.getElementsByClassName('swiper-slide');
        for(var index = 0; index< swiperSlide.length; index++){
          swiperSlide[index].addEventListener('mouseover', (e)=>{ productSlider.zoom.in()});
          swiperSlide[index].addEventListener('mouseout', (e)=>{ productSlider.zoom.out()});
        }
      }
    }else if(sliderElement.classList.contains('quickshop-slider')){
      quickShopSlider = slider;
    }
...
class customVideo extends HTMLElement {
  constructor() {
    ...
    this.container = this.closest('.full-video-layout');
    this.customVideoContainer = this.closest('.custom-video-banner');
    this.videoModal = document.getElementById('PopupModal-video');
    this.triggerVideo = this.querySelector('[data-trigger-video]');
    this.pauseVideo = this.querySelector('[data-pause-video]');
    this.fullWidthTriggerBtn = (this.container) ? this.container.querySelector('[data-trigger-videobtn]') : null;
    ...
    if(this.placementType == 'popup') this.videoSection = this.videoModal.querySelector('.video-section');

    if(window.screen.width <= 767 && this.dataset.mobilevideo){
      this.videoID = this.dataset.mobilevideo;
    }
    this.bindEvents();
  }

  bindEvents(){
    this.triggerVideo.addEventListener('click', this.toggleView.bind(this));
    if(this.placementType == 'grid') this.addEventListener('focusout', this.focusOut.bind(this));

    if(this.fullWidthTriggerBtn) {
      this.fullWidthTriggerBtn.addEventListener('click', (e) => {
        e.preventDefault();
   ...
   */
  focusOut(event){
    if(this.placementType == 'popup' && this.classList.contains('show--video')) this.toggleView();
  }

  toggleView(){
    let isOpen = this.classList.contains('show--video');
    if(isOpen){
      this.videoSection.innerHTML = '';
      this.classList.remove('show--video');
      if(this.container) this.container.classList.remove('playing--video');
    }else{
      const videoElement = this._buildVideo();
      this.videoSection.appendChild(videoElement);
      this.classList.add('show--video');
      if(this.placementType == 'popup'){ openModal(this.videoModal, this.triggerVideo)}
      else if(this.container){ this.container.classList.add('playing--video');}

      // Inject YouTube API scrip
      if(this.type == 'youtube' && window.ytAPILoaded == false){
      ...
      if(this.type == 'local' || this.type == 'youtube'){
        this.playPauseEvent(videoElement);
      }
    }
  }
  ...
  _buildVideo(){
    ...
    if(this.type == 'local'){
      ...
      video.setAttribute( "src", "https://www.youtube.com/embed/"+ this.videoID +`?rel=0&showinfo=0&autoplay=${this.dataset.autoplay}&mute=${this.dataset.muted}&controls=${this.dataset.controls}&loop=${this.dataset.loop}&playlist=${this.videoID}` );
    }else if(this.type == 'vimeo'){
      video = document.createElement( "iframe" );
      video.setAttribute( "allowfullscreen", "" );
      video.setAttribute( "frameborder", "0" );
      video.setAttribute( "src", "https://player.vimeo.com/video/"+ this.videoID + `?rel=0&showinfo=0&autoplay=${this.dataset.autoplay}&mute=${this.dataset.muted}&controls=${this.dataset.controls}&loop=${this.dataset.loop}&playlist=${this.videoID}`);
    }
    return video;
  }

  playPauseEvent(video){
    const _this = this;
    // let player = new YT('video', {
    //   events: {
    ...
    // });
    if(_this.pauseVideo){
      _this.pauseVideo.addEventListener('click', (e)=> {
        video.play();
      });
    }
    ...
    if(this.type == 'local'){
      video.onplay = function(e) {
        if(_this.customVideoContainer){
          _this.customVideoContainer.classList.add('video--playing');
          _this.customVideoContainer.classList.remove('video--paused');
        }
      };
      video.onpause = function(e) {
        if(_this.customVideoContainer){
          _this.customVideoContainer.classList.add('video--paused');
          _this.customVideoContainer.classList.remove('video--playing');
        }
      };
      video.onended = function(e) {
        _this.customVideoContainer.classList.remove('video--playing', 'video--paused');
        _this.classList.remove('show--video');
      };
    }
  }
}
...
/**
   * Manage KlaviyoForm submissions
   *
   */
 class customKlaviyo extends HTMLElement {
  constructor() {
    super();
    this.form = this.querySelector('form');
    this.successMsg = this.getAttribute('data-successmsg');
    this.subscribed = this.getAttribute('data-subscribed')
    this.responseInput = this.querySelector('[data-final-result]');

    this.form.addEventListener('submit', this.onSubmitHandler.bind(this));
  }

  /**
   * Klaviyo Form Submit event
   *
   ...
   */
   onSubmitHandler(evt) {
    ...
    // Validation Code
    const listID = this.form.querySelector('[name="g"]').value;
    const emailID = this.form.querySelector('[name="email"]').value;
    if(emailID.trim().length <= 0){
      this.responseInput.innerHTML = 'Empty Email';
      this.manageResponseText(this.responseInput);
      return;
    }else if(!validateEmail(emailID)) {
      this.responseInput.innerHTML = 'Invalid Email';
      this.manageResponseText(this.responseInput);
      return;
    }else if(listID == null || listID.length <= 0) {
      this.responseInput.innerHTML = 'Invalid List ID';
      this.manageResponseText(this.responseInput);
      return;
    }
    ...
    .then((parsedState) => {
      if(parsedState.success == false){
        const errortext = parsedState.errors[0];
        this.responseInput.innerHTML = errortext;
        this.manageResponseText(this.responseInput);
      }else if(parsedState.success == true && parsedState.data.is_subscribed == true){
        this.responseInput.innerHTML = this.subscribed;
        this.manageResponseText(this.responseInput);
      }else{
        this.form.querySelector('[name="email"]').value = '';
        this.responseInput.innerHTML = this.successMsg;
        this.manageResponseText(this.responseInput);
      }
    })
    .catch((e) => {
...
customElements.define("custom-klaviyo", customKlaviyo);

/**
   * Manage LazySizes If not managed through default JS
   *
   */
(function manageLazySizes() {
  window.addEventListener('resize', ()=>{
    const lazySizeImg = document.querySelectorAll('.lazyload');
    const windowWidth = window.innerWidth;
    lazySizeImg.forEach((img) => {
